...
def main():
    ...
    scale = Scale.from_text('C ionian')
    ctx = Context(tuning, scale)

    shape = get_caged_shape(ctx, CagedPosition.E)

    print_shape(ctx, shape)
    print()

    for exercise in included_exercises:
        positions = exercise_from_shape_and_pattern(shape, exercise.pattern)
